...
//thisChunkNum is the one currently being written
// UNLESS between block-complete and Init
WriteBlocks:
   Init:
      FindNextAvailableChunk_IncludingThis:
         while(!chunkAvailable(thisChunkNum))
            thisChunkNum++
      //endChunk = findFragmentEnd(thisChunkNum)
      //             endChunk = thisChunkNum
      //             while(chunkAvailable(endChunk))
      //                endChunk++
      //             endChunk--
      START
   Start:
      SD_WriteMultiple(startBlockFromChunk(thisChunkNum))
      WRITE
   Write:
      ...
   Block Complete:
      ...
      if(blockNum_inThisChunk == numBlocksPerChunk)
         ...
         if(!chunkAvailable(thisChunkNum)) //FindNextAvailableChunk_AFTERThis(
            STOP
         else
            WRITE
   Stop:
...
#define SAMPLE_TCNT  1//5//6//64//15   //6
   ...
   #define KBBUFFER_SIZE   20
      ...
      #define MEMO_TIMEOUT (2500000)   //10sec
            ...
            //      even if thisChunkNum increases in the midst of a
...
//       thisChunkNum = 0xff, so increment it to 0
//       findNextUsableChunk(++thisChunkNum)
//          may return 0, as well
//  3) At the end of a fragment e.g.
//       thisChunkNum = 5, increment it to 6 (which is known to be used)
//       findNextUsableChunk(++thisChunkNum)
...
static __inline__ uint8_t findNextUsableChunk(uint8_t startingChunkNum)
{
   ...
   while(0 != eeprom_read_byte((uint8_t *)((uint16_t)chunkNum)))
   {
      ...
      //if(thisChunkNum >= (USAGETABLE_SIZE))
      // thisChunkNum = 0;
      //else
      // thisChunkNum++;
      chunkNum++;
...
//
// e.g.
//    uint8_t chunkTemp = thisChunkNum;
...
/* LIES ALL LIES!!! 
   ...
   IOW: No Chunk-Bypassing Bug.
         ...
         //while(0 != eeprom_read_byte((uint8_t *)thisChunkNum))
...
#warning "thisLoopNum == 0xff is poorly handled."
         ...
         uint8_t utVal;
         while( (0 != (utVal=eeprom_read_byte((uint8_t *)thisChunkNum)))
               && (utVal != thisLoopNum) )
*/
/* 
...
*/
            ...
            thisChunkNum = findNextUsableChunk(thisChunkNum);
#if(defined(PRINT_CHUNKADVANCE) && PRINT_CHUNKADVANCE)
            sprintf_P(stringBuffer, PSTR("cn:0x%"PRIx8), thisChunkNum);
            puat_sendStringBlocking(stringBuffer);
#endif
            //Now thisChunkNum IS available...
         
            //Calculate the block number
            uint32_t chunkStartBlock;
            chunkStartBlock = startingBlockNumFromChunk(thisChunkNum);
      ...
      case WRITE_BUSY_STATE:
         {
            ...
            //{
               ...
               if(blockNum_inThisChunk == numBlocksPerChunk)
               {
                  ...
                  else if(0 == eeprom_read_byte((uint8_t *)thisChunkNum))
                  {
                  ...
                  else
                  {
                     ...
                     // available chunk
                     // (thisChunkNum will be updated accordingly
         ...
         // } // End of No Longer Busy case
            ...
            if(usageTableUpdateState == 3)
            {
               ...
               else
               {
                  ...
                  //reStartBlock = (uint32_t)thisChunkNum 
                  //                * numBlocksPerChunk;
                  // Heh, there woulda been a bug allowing usage-table
                  //  overwriting, with the old method!
                  //  Nah, 'cause thisChunkNum==0 wouldn't be the case
                  //    if we're here (mid-chunk)
                  reStartBlock = startingBlockNumFromChunk(thisChunkNum);
